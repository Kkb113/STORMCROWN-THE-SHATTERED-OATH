...
export class GameRenderer {
  constructor(canvas,settings){
    ...
    this.bloom=new UnrealBloomPass(new THREE.Vector2(innerWidth,innerHeight),.65,.5,.78);this.composer.addPass(this.bloom);
    ...
    canvas.addEventListener('webglcontextlost',event=>{event.preventDefault();this.contextLost=true;this.onError?.('The graphics context was interrupted. Your last checkpoint is safe. Reload to reconnect to the renderer.');});
  }
  async load(progress){
    await this.materials.load(progress);this.scene.environment=this.materials.environment;this.scene.environmentIntensity=.62;
  ...
  setSimulation(sim,{menu=false}={}){
    ...
    for(const a of this.actors.values())this.characters.destroy(a);this.actors.clear();this.sim=sim;this.mode=menu?'menu':'game';
    ...
    this.hemi.color.set(r.ambient);this.hemi.groundColor.set(r.fog);this.hemi.intensity=sim.mission.region===2?1.6:1.5;
    this.key.color.set(r.key);this.key.intensity=sim.mission.region===1?3.0:3.6;this.fill.color.set(r.fill);this.fill.intensity=sim.mission.region===2?.8:1.1;
    ...
    this.sim.on('switch',()=>{this.lastSwitch=this.time;});
  }
  ...
  updateCamera(dt,snap=false){
    if(!this.sim)return;
    const p=this.sim.activeHero,y=this.sim.world.heightAt(p);
    ...
    offset.set(Math.sin(yaw)*32,32.5,Math.cos(yaw)*32).multiplyScalar(zoom);
    ...
    this.key.position.copy(this.target).add(new THREE.Vector3(-24,42,10));this.key.target.position.copy(this.target);this.key.target.updateMatrixWorld();
    this.fill.position.copy(this.target).add(new THREE.Vector3(20,15,-23));this.fill.target.position.copy(this.target);this.fill.target.updateMatrixWorld();
  }
  updateActors(dt){
    ...
    for(const e of [...this.sim.party,...this.sim.enemies,...this.sim.civilians,...this.sim.constructs,...this.sim.figures]){
      ...
      this.characters.animate(actor,e,this.actorTime||0,dt);actor.group.position.y+=this.sim.world.heightAt(e);
  ...
  render(dt){
    if(!this.ready||!this.sim||this.contextLost)return;const frameDt=Math.max(.001,dt);dt=Math.min(.1,Math.max(0,dt));this.time+=dt;if(!this.presentationPaused)this.actorTime=(this.actorTime||0)+dt;
    this.updateCamera(dt);this.updateActors(dt);this.environment.update(this.time,dt,this.sim.activeHero,this.camera,this.vfx.flash);this.weather.update(this.time,dt,this.sim.activeHero,this.sim.world.heightAt(this.sim.activeHero));this.vfx.update(this.time,dt,this.environment);
  ...
  dispose(){removeEventListener('resize',this.onResize);for(const a of this.actors.values())this.characters.destroy(a);this.vfx?.dispose();this.weather?.dispose();this.environment?.dispose();this.characters?.dispose();this.materials.dispose();this.ao.dispose();this.composer.dispose();this.renderer.dispose();}
}
