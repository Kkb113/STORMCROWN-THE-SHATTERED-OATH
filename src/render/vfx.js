...
import { flatRing, shardGeometry } from './geometry.js';

const colorOf=e=>ELEMENTS[e]?.hex||0xd3bd9c;
const UP=new THREE.Vector3(0,1,0),V=new THREE.Vector3(),M=new THREE.Matrix4(),Q=new THREE.Quaternion(),S=new THREE.Vector3();
...
export class VFX {
  constructor(scene,materials,camera,settings){
    ...
    this.maxParticles=2200;this.particles=Array.from({length:this.maxParticles},()=>({life:0}));
    ...
    const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.BufferAttribute(this.particlePositions,3));g.setAttribute('pColor',new THREE.BufferAttribute(this.particleColors,3));g.setAttribute('pInfo',new THREE.BufferAttribute(this.particleInfo,3));
    ...
    for(let i=0;i<this.maxDebris;i++){M.makeScale(0,0,0);this.debrisMesh.setMatrixAt(i,M);}
    this.telegraphs=new Map();this.zoneMeshes=new Map();this.makeFire();
    this.projectileMesh=new THREE.InstancedMesh(new THREE.ConeGeometry(.14,.9,6),materials.glow(0xffffff,2),120);this.projectileMesh.frustumCulled=false;this.projectileMesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);this.group.add(this.projectileMesh);
  ...
  clear(){
    ...
    for(const h of this.telegraphs.values()){h.removeFromParent();h.geometry.dispose();h.material.dispose();}this.telegraphs.clear();
    for(const m of this.zoneMeshes.values()){m.removeFromParent();m.geometry.dispose();m.material.dispose();}this.zoneMeshes.clear();this.flash=0;
  }
  floor(p){return this.sim?.world.heightAt(p)||0;}
  emitParticle(x,y,z,vx,vy,vz,color,size=.25,life=.6,gravity=5,smoke=false){
    const p=this.particles[this.particleCursor++%this.maxParticles];Object.assign(p,{x,y,z,vx,vy,vz,color:new THREE.Color(color),size,life,total:life,gravity,smoke});
  }
  burst(x,z,count,color,radius=1,speed=7,y=1){
    count=Math.ceil(count*this.settings.particles);const ground=this.floor({x,z});
  ...
  debrisBurst(x,z,count=18,radius=3,color=0x8b8098){
    const ground=this.floor({x,z});for(let i=0;i<count*this.settings.particles;i++){
  ...
  effect(e){
    ...
    if(type==='wall'){this.lightning({x:e.x-4,y:0,z:e.z},{x:e.x+4,y:8,z:e.z},e.color,.2,4,e.life||1);return;}
  ...
  makeFire(){
    const g=new THREE.PlaneGeometry(1.5,2.4);g.translate(0,1.05,0);
    const m=new THREE.ShaderMaterial({transparent:true,depthWrite:false,side:THREE.DoubleSide,blending:THREE.AdditiveBlending,uniforms:{time:{value:0},tint:{value:new THREE.Color(0xffad54)}},vertexShader:`varying vec2 vUv;varying float vSeed;void main(){vUv=uv;vSeed=instanceMatrix[3].x+instanceMatrix[3].z;vec4 origin=modelViewMatrix*instanceMatrix*vec4(0.,0.,0.,1.);origin.xy+=position.xy;gl_Position=projectionMatrix*origin;}`,fragmentShader:`varying vec2 vUv;varying float vSeed;uniform float time;uniform vec3 tint;${hashNoise}void main(){vec2 p=vUv;float n=noise(vec2(p.x*5.+vSeed,p.y*7.-time*4.));float w=(1.-p.y)*.5;float flame=smoothstep(w+.10,w-.15,abs(p.x-.5)+(n-.5)*.20);flame*=smoothstep(0.,.13,p.y)*smoothstep(1.,.2,p.y);float hot=pow(flame,3.)*(1.-p.y);vec3 col=tint*flame*2.3+vec3(1.1,.8,.5)*hot;gl_FragColor=vec4(col,flame*.8);}`});
    this.fire=new THREE.InstancedMesh(g,m,32);this.fire.frustumCulled=false;this.group.add(this.fire);
  }
  createTelegraph(h){
    const material=new THREE.ShaderMaterial({transparent:true,depthWrite:false,side:THREE.DoubleSide,uniforms:{color:{value:new THREE.Color(h.absorb?0x8d9fff:h.element==='frost'?0xe896bf:0xff705a)},progress:{value:0},shape:{value:h.shape==='rect'?2:h.shape==='ring'?1:h.shape==='cone'?3:0},inner:{value:h.inner?(h.inner/(h.radius||1)):0},arc:{value:h.arc||2},time:{value:0}},
    ...
    const width=h.shape==='rect'?h.width:(h.radius||4)*2,length=h.shape==='rect'?h.length:(h.radius||4)*2;
    const mesh=new THREE.Mesh(new THREE.PlaneGeometry(width,length),material);mesh.rotation.x=-Math.PI/2;mesh.renderOrder=2;this.group.add(mesh);this.telegraphs.set(h.id,mesh);return mesh;
  }
  updateTelegraphs(time){
    const ids=new Set();for(const h of this.sim.hazards){if(distance(h,this.sim.activeHero)>55)continue;ids.add(h.id);let m=this.telegraphs.get(h.id);if(!m)m=this.createTelegraph(h);m.position.set(h.x,this.floor(h)+.08,h.z);m.rotation.set(-Math.PI/2,0,-(h.angle||0));m.material.uniforms.progress.value=h.warn>0?1-h.warn/h.totalWarn:1;m.material.uniforms.time.value=time;}
    for(const [id,m] of this.telegraphs)if(!ids.has(id)){m.geometry.dispose();m.material.dispose();m.removeFromParent();this.telegraphs.delete(id);}
  }
  updateZones(time,dt){
    const ids=new Set();for(const z of this.sim.zones){if(distance(z,this.sim.activeHero)>60)continue;ids.add(z.id);let mesh=this.zoneMeshes.get(z.id);
      ...
      const count=Math.ceil((['cyclone','vortex','hurricane'].includes(z.kind)?4:2)*this.settings.particles);
      if(this.clock%2===0)for(let i=0;i<count;i++){
  ...
  update(time,dt,environment){
    ...
    this.flashLight.intensity=this.flash*1400*(this.settings.flashes||0);
    ...
    const positions=this.particlePositions,colors=this.particleColors,info=this.particleInfo;
    for(let i=0;i<this.maxParticles;i++){
      const p=this.particles[i];if(p.life>0){p.life-=dt;p.vy-=p.gravity*dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.z+=p.vz*dt;positions[i*3]=p.x;positions[i*3+1]=p.y;positions[i*3+2]=p.z;p.color.toArray(colors,i*3);info[i*3]=p.size;info[i*3+1]=Math.max(0,p.life/p.total);info[i*3+2]=p.smoke?1:0;}else{info[i*3]=0;info[i*3+1]=0;}
    }
    this.points.geometry.attributes.position.needsUpdate=true;this.points.geometry.attributes.pColor.needsUpdate=true;this.points.geometry.attributes.pInfo.needsUpdate=true;
    this.points.material.uniforms.scale.value=globalThis.innerHeight*.62;
    for(let i=0;i<this.maxDebris;i++){
      const d=this.debris[i];if(d.life>0){d.life-=dt;d.vy-=dt*18;d.x+=d.vx*dt;d.y+=d.vy*dt;d.z+=d.vz*dt;if(d.y<d.ground+.07){d.y=d.ground+.07;d.vy=Math.abs(d.vy)*.18;d.vx*=.85;d.vz*=.85;}d.rx+=dt*3;d.rz+=dt*4;Q.setFromEuler(new THREE.Euler(d.rx,0,d.rz));S.setScalar(d.size*Math.min(1,d.life*2));M.compose(V.set(d.x,d.y,d.z),Q,S);}else M.makeScale(0,0,0);this.debrisMesh.setMatrixAt(i,M);
    }this.debrisMesh.instanceMatrix.needsUpdate=true;
    const projectiles=this.sim.projectiles.slice(0,120);this.projectileMesh.count=projectiles.length;
    projectiles.forEach((p,i)=>{Q.setFromUnitVectors(UP,new THREE.Vector3(Math.sin(p.angle),0,Math.cos(p.angle)));S.setScalar(p.visual==='spear'?1.5:p.visual==='arrow'?.8:1);M.compose(V.set(p.x,p.y+this.floor(p),p.z),Q,S);this.projectileMesh.setMatrixAt(i,M);this.projectileMesh.setColorAt(i,new THREE.Color(colorOf(p.element)));if(this.clock%3===0)this.emitParticle(p.x,p.y+this.floor(p),p.z,0,0,0,colorOf(p.element),.12,.2,0);});
    this.projectileMesh.instanceMatrix.needsUpdate=true;if(this.projectileMesh.instanceColor)this.projectileMesh.instanceColor.needsUpdate=true;
    ...
    fires.forEach((t,i)=>{M.makeTranslation(t.x,t.y,t.z);this.fire.setMatrixAt(i,M);if(this.clock%7===0)this.emitParticle(t.x,t.y+.5,t.z,(Math.random()-.5)*.3,1.4+Math.random(),(Math.random()-.5)*.3,0xffb869,.065,1.3,-.2);});this.fire.instanceMatrix.needsUpdate=true;
  }
  dispose(){this.clear();this.group.removeFromParent();for(const m of [this.points,this.debrisMesh,this.fire,this.projectileMesh]){m.geometry.dispose();m.material.dispose();}this.flashLight.removeFromParent();this.boltMaterial.dispose();this.basicGlow.dispose();}
}
